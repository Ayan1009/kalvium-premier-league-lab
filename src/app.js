...
function filterByDebut(year) {
  return players.filter((e) => e.debut === year);
}
...
function filterByAward(awardName) {
  return players.filter((e) =>
    e.awards.some((e) => e.name === awardName)
  );
}
...
function filterByAwardxTimes(awardName, noOfTimes) {
  return players.filter(
    (e) =>
      e.awards.filter((e) => e.name === awardName).length === noOfTimes
  );
}
...
function filterByAwardxCountry(awardName, country) {
  ...
  return filtered.filter((e) => e.country === country);
}
...
function filterByNoOfAwardsxTeamxAge(noOfAwards, team, age) {
  return players.filter(
    (e) =>
      e.awards.length >= noOfAwards && e.team === team && e.age < age
  );
}
...
return players.filter(e=>e.age>age).sort((a,b)=>a.name.localeCompare(b.name))
}
